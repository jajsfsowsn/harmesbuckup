...
§
User's Vercel account: mehrdad6. Token: [REDACTED_VERCEL_TOKEN]. Main project: mvpndeployer-deploy (VPN panel deployer). Railway projects use 3x-ui-Upgrade fork. User prefers xPaddingBytes: '0-0' (no padding) for speed. Uses NapsterNetV app on mobile for VPN.
§
...
§
User uses NapsterNetV (Npv Tunnel) app on mobile for VPN. .npvt files are White-Box AES CTR encrypted (NPVT1 format). User has panels: 3x-ui-upgrade-production-e554 and d328 on Railway.