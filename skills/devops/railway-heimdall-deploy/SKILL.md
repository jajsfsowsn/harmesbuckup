...
11. **3x-ui null stream settings cause TLS errors** — When an inbound's `stream` field is `null` (empty/unset), Xray may use default settings which include TLS with empty certificates. Error: "Failed to build TLS config > both file and bytes are empty". Fix: explicitly set `stream: {"network": "tcp", "security": "none"}` for all inbounds that don't need TLS. Check via API: if `stream` is `null` for any inbound, update it.
...
### Standalone Fallback (RECOMMENDED)
The Ourenus page is a React SPA that fetches data from the panel's API (`/info`, `/configs`). If the sub server (port 2096) isn't running, the page shows only a loading spinner with "خطای اتصال به سرور" error.

**Best approach: Create a standalone HTML page** with the VLESS config hardcoded. No React, no API calls, no build step. See `templates/standalone-sub-view.html`.

**Avoid: Fetch/XHR Override** — The Ourenus React app uses **axios** internally, which bypasses raw `XMLHttpRequest.prototype.send` and `window.fetch` overrides. Users will still see the connection error. Only use this if the sub server is actually running.
...
- **`restartXrayService` POST may return empty** — check if Xray actually restarted by querying `server/status` afterward (xray.state should be "running").
