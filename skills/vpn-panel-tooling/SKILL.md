...
#### ⚠️ CRITICAL: `stream` vs `streamSettings` field naming
Different 3x-ui forks use DIFFERENT field names for stream settings:
- **`inbounds/list`** returns `stream` (lowercase, some forks return null)
- **`inbounds/get/{id}`** returns `streamSettings` (camelCase)
- **`inbounds/update/{id}`** accepts **BOTH** `stream` and `streamSettings`
- **3x-ui-multi v3.6.0**: update ONLY saves if you use `streamSettings`. Using `stream` silently fails (returns success but settings not persisted).
- **Always check** after update: GET the inbound and verify `streamSettings` is populated, not null.

#### Railway TLS Pattern (CRITICAL)
On Railway, TLS is terminated at the edge proxy — Xray never sees TLS:
- **Server inbound**: `security: "none"` (Xray receives plain HTTP from Railway proxy)
- **Client config**: `security: "tls"` (client connects to Railway domain with TLS on port 443)
- If you set `security: "tls"` on the server inbound, Xray fails with "both file and bytes are empty" because there are no certificates
- **WebSocket + TLS on Railway**: inbound uses `network: "ws", security: "none"`, client uses `network: "ws", security: "tls"`
...
- **Frustration signals:** "اوسکول", "چرا کار نمیکنی", "بکن دیگه", "انجام بده خودت", "کونی بابا درستش کن دیگه خارکسده" = user wants action NOW. When user sends frustration + "بازم ارور داد" (error again), IMMEDIATELY stop explaining and start debugging/fixing. Do NOT suggest alternatives or ask them to do something — just fix it.
...
- **3x-ui null stream settings cause TLS errors** — When an inbound's `stream`/`streamSettings` field is `null` (empty/unset), Xray may use default settings which include TLS with empty certificates. Error: "Failed to build TLS config > both file and bytes are empty". Fix: explicitly set `streamSettings: {"network": "tcp", "security": "none"}` for all inbounds that don't need TLS. On Railway, ALL inbounds should use `security: "none"` because Railway terminates TLS at the edge proxy. Check via API: if `streamSettings` is `null` for any inbound, update it.
- **Systematic Xray TLS debugging (disable-all-then-enable)** — When multiple inbounds have TLS issues, fix them one at a time: (1) Disable ALL inbounds via `setEnable/{id}` with `enable: false`, (2) Restart Xray → confirm it runs, (3) Enable inbounds one by one, restarting after each, (4) When Xray breaks, that inbound is the culprit — fix its stream settings, (5) Continue until all inbounds are enabled and Xray runs. The error message always names the specific tag (e.g., `in-8081-tcp`).
- **API field naming: `stream` vs `streamSettings`** — The `inbounds/list` endpoint may return `stream` (lowercase) or null. The `inbounds/get/{id}` endpoint returns `streamSettings` (camelCase). For 3x-ui-multi v3.6.0, the update endpoint ONLY correctly saves stream settings when you use `streamSettings` as the field name — using `stream` silently fails (returns success but settings remain null). After updating, always GET the inbound to verify `streamSettings` is populated. For other forks, test which field name works.
...
## 3x-ui-multi Panel (v3.6.0) API Reference
The 3x-ui-multi panel (used for multi-location deployments) has slightly different API behavior from Heimdall.

### Authentication
Login works with **form-encoded** data (no CSRF required):
```python
# Form-encoded login (simpler, no CSRF needed)
r = s.post(f"{base}/login", data={"username": "admin", "password": "admin"})

# JSON login with CSRF (also works)
r = s.get(f"{base}/csrf-token")
csrf = r.json()["obj"]
r = s.post(f"{base}/login", headers={"X-CSRF-Token": csrf, "Content-Type": "application/json"},
...
| Endpoint | Method | Purpose |
|----------|--------|---------|
| `/managepanel/login` | POST | Login (form or JSON) |
| `/managepanel/csrf-token` | GET | CSRF token |
| `/panel/api/inbounds/list` | GET | List all inbounds |
| `/panel/api/inbounds/get/{id}` | GET | Get inbound details |
| `/panel/api/inbounds/add` | POST | Create inbound |
| `/panel/api/inbounds/update/{id}` | POST | Update inbound (use `streamSettings`!) |
| `/panel/api/inbounds/setEnable/{id}` | POST | Enable/disable inbound |
| `/panel/api/server/status` | GET | Server status + Xray state |
| `/panel/api/server/restartXrayService` | POST | Restart Xray |
| `/panel/api/server/xraylog/{count}` | POST | Get Xray logs |

### Multi-Location Pattern
3x-ui-multi creates one inbound per country/location. Each inbound uses the same server but different ports:
- Direct: 8080, Canada: 8081, Turkey: 8082, Germany: 8083, etc.
- All inbounds share the same Railway domain
- Client connects to port 443 (Railway edge), traffic routes to the correct inbound port

### VLESS + WS + TLS Config (Railway)
For client configs with WebSocket + TLS on Railway:
```json
{
  "streamSettings": {
    "network": "ws",
    "security": "tls",
    "tlsSettings": {"serverName": "DOMAIN.up.railway.app"},
    "wsSettings": {"path": "/de", "host": "DOMAIN.up.railway.app"}
  }
}
```
Server inbound uses `security: "none"` (Railway handles TLS). Client config uses `security: "tls"`.
...
- ⚠️ **Ourenus is a dynamic React SPA** — it fetches data from `/info` and `/configs` API endpoints. If the sub server (port 2096) is not running, the page shows a loading spinner with "خطای اتصال به سرور" error.

### Standalone HTML (RECOMMENDED — most reliable)
When the sub server isn't running, create a **pure HTML/CSS/JS page** with the VLESS config hardcoded. No React, no API calls, no build step. This is the most reliable approach. See `templates/standalone-sub-view.html` for a working template with QR code, server info grid, copy button, and app download links.

### Fetch/XHR Override for Ourenus (UNRELIABLE — avoid)
Injecting fetch/XHR overrides into the pre-built Ourenus HTML **does not work reliably** because:
- The React app uses **axios** (not raw fetch/XHR) for API calls
- Axios creates its own XMLHttpRequest internally, bypassing prototype overrides
- Even overriding `XMLHttpRequest.prototype.open/send` fails because axios intercepts at a different layer
- Users see "خطای اتصال به سرور" error despite the override being injected
- **If you must use Ourenus**, ensure the sub server (port 2096) is running. Otherwise, use the standalone template.
...
- **`server/status` GET works** — returns full CPU, memory, disk, Xray state/version/errorMsg. Use this for checking if Xray is running.
- **`restartXrayService` POST may return empty body** — it executes but response is `{"output":"","exit_code":0}`. Verify restart by querying `server/status` afterward.

## Reference Files
- `references/xray-tls-debugging.md` — Systematic debugging technique for Xray TLS errors: disable-all-then-enable-one-by-one approach, API field naming quirks (`stream` vs `streamSettings`), Python script template.
